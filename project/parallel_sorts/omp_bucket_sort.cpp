...
void bucket_sort(vector<int>::iterator vec_beg, vector<int>::iterator vec_end) {
    ...
    int* counters = new int[psize + 1]();
    counters[0] = 0;
    std::vector<std::vector<int>> buckets(psize);

    int n = vec_end - vec_beg;

    int m = RAND_MAX;
    int interval = m / psize;
...
#pragma omp parallel for schedule(guided) shared(counters, v)
    for (int i = 0; i < psize; ++i)
    {
...
int main(int argc, char* argv[]) {
    ...
    unsigned int start_time_bucket = omp_get_wtime();
    bucket_sort(numbers_bucket.begin(), numbers_bucket.end());
    unsigned int end_time_bucket = omp_get_wtime();
    int time_bucket = end_time_bucket - start_time_bucket;

    std::cout << "Parallel finished" << '\n';
    std::cout << "len = " << len << '\n';
    std::cout << "num threads = " << num_threads << '\n';
    std::cout << "Time of parallel omp bucket sort = " << time_bucket << "\n";
